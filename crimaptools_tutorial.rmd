...
The `.gen` file must now be run through *prepare* to produce the field used for linkage mapping and crossover estimation. This can be done using the function `run_crimap_prepare`. NB. In non-Windows systems at the moment, the exectuable path must be specified relative to the `.gen` file with the argument `crimap.path = `.
